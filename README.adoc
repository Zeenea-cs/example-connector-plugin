...
----
dependencies {
    val jarFiles = fileTree("lib") { include("*.jar") }  <1>
    compileOnly(jarFiles)               <2>
    testImplementation(jarFiles)        <3>
    compileOnly(libs.pf4j)              <4>
    testImplementation(libs.pf4j)       <5>
    annotationProcessor(libs.pf4j)      <6>
    implementation(platform(libs.jackson.bom))  <7>
...
So the dependecy can be added as a file in the project.
You will find the jar in the public-connector-sdk-_version_.jar in the folder lib of the scanner.
<2> The public-connector-sdk library is added in compile-only scope.
<3> The public-connector-sdk library is added in test scope.
<4> The PF4J library is added in compile-only scope.
<5> The PF4J library is added in test scope.
<6> The PF4J library is added in annotation processing scope.
This is required to create the _META-INF/extensions.idx_ index.
<7> Other dependency are declared as usually.

Because the SL4J libraries can come from any dependeny or recursive dependency, we exclude them from the distribution when packaging:
